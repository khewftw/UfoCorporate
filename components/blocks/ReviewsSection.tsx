...
export function ReviewsSection() {
  return (
    <section className={`w-full ${ds.sectionSpacing || "py-16 md:py-24"}`}>
      ...
      {/* Сетка карточек */}
      <div className={`grid grid-cols-1 md:grid-cols-2 ${ds.gridGap || "gap-6"}`}>
        {cases.map((item) => (
          <article
            key={item.clientName}
            className="flex flex-col rounded-[32px] bg-white p-6 md:p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/50 transition-all duration-300 hover:shadow-md hover:-translate-y-1"
          >
            {/* Шапка карточки */}
            <header className="mb-6 flex flex-wrap items-start justify-between gap-4">
              <div>
                <h3 className="text-xl font-bold text-[#111827]">{item.clientName}</h3>
                <p className="mt-1 text-sm font-medium text-[#6B7280]">{item.meta}</p>
              </div>
              <a 
                href="#" 
            ...
            <div className="mb-8 space-y-3">
              <p className="text-sm font-medium text-[#6B7280]">
                Долг при обращении:{" "}
                <span className="text-lg font-bold text-[#111827]">{item.debtAtStart}</span>
              </p>
              <div className="inline-flex items-center gap-2 rounded-xl bg-[#F7F8FA] px-3 py-2 text-sm font-medium text-[#6B7280]">
                После списания: <span className="font-bold text-[#111827]">0 руб.</span>
              </div>
            </div>
            ...
            <footer className="mt-auto flex flex-wrap items-center justify-between gap-4 border-t border-gray-100 pt-6">
              ...
              <button
                type="button"
                className="rounded-xl border border-gray-200 bg-white px-5 py-3 text-sm font-bold text-[#111827] transition-all hover:border-gray-300 hover:bg-gray-50 active:scale-95"
              >
                Перейти к отзыву
              </button>
            </footer>
